...
func New(opts ...Option) (Store, error) {
	...
	headers["X-HuggingFace-Api-Key"] = cfg.HuggingFace.APIToken
	...
	log.Debug().Type("opts", s).Msg("using custom New Weaviate store...")
...
func (s Store) SimilaritySearch(
	...
	log.Debug().Msgf("threshold: %v", opts.ScoreThreshold)
	filter := s.getFilters(opts)
	whereBuilder, err := s.createWhereBuilder(nameSpace, filter)
	if err != nil {
		return nil, err
	}
	client := s.client.GraphQL()
	nearText := client.NearTextArgBuilder().
